...
binanceError_t binance::Market::getLastFundingRate(Json::Value &json_result, const char *symbol, double &fundingRate)
{
	binanceError_t status = binanceSuccess;
...
binanceError_t binance::Market::getServerTime(Json::Value &json_result, double &serverTime)
{
	binanceError_t status = binanceSuccess;
	...
	else
	{
		try
		{
			...
			json_result.clear();
			reader.parse(str_result, json_result);
			serverTime = stod(json_result["serverTime"].asString());
			CHECK_SERVER_ERR(json_result);
		}
		catch (exception &e)
		{
