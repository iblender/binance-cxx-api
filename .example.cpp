...
{	
	...
	klinesCache[start_of_candle]["q"] = atof(json_result["k"]["q"].asString().c_str());
...
int main()
{
	...
	// Klines / CandleStick
	BINANCE_ERR_CHECK(market.getFundingRate(result, "BTCUSDT", 0, 0, 10));
 	...
 	for (Json::Value::ArrayIndex i = 0 ; i < result.size() ; i++)
 	{
		...
		klinesCache[start_of_candle]["q"] = atof(result[i][4].asString().c_str());
 		klinesCache[start_of_candle]["v"] = atof(result[i][5].asString().c_str());
 	}

 	print_klinesCache();
 		
 	// FundingRate update via websocket
 	Websocket::init();
